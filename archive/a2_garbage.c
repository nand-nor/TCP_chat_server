...
struct sock_cli {
        int sockfd;
        char ID[20];
        //struct sock_cli *next;
};

struct cli_ID {
        char ID[20]; //client username
                ...
                if(FD_ISSET(i, listen_set)){
                        ...
                        if(i == master_sock){
                                ...
                                do {
                                        ...
                                        printf("Client has nothing coming in (i.e. message header was already sent),\n and nothing in the message queue to receive.\n So read whatever request they want to send...\n");
...
int parse_resp(int sock, struct header_C *resp, char *buffer) {
        ...
        printf("parse_resp check: client %s has socket %d\n", resp->source, sock);
        ...
        printf("parse_resp check: size and ID: %u %u\n", size, id);        
        int ret_val = 0, n;
        switch(type) { 
                case 1:
                        printf("its a hello\n");
                        ...
                        if(client_check(resp->source)){
                                printf("returning error 8 -- client already present\n");
                        ...
                        memcpy(&new_cli->ID,resp->source,20);
                        new_cli->ID[19] = '\0';
                        new_cli->active = 1;
                        new_cli->sockfd = sock;
                        ...
                        put(new_cli);
                        struct sock_cli *c = malloc(sizeof(*c));
                        memcpy(&c->ID,resp->source,20);
                        c->ID[19] = '\0';
                        c->sockfd = sock;
                        //c->next = NULL;
                        CLIENTS->queue[sock] = c;
                        ++CLIENTS->size;
                       ...
                       //         ++CLIENTS->size;
                      // }else {
                      //          struct sock_cli *ptr = CLIENTS->queue;
                      ///          c->next = ptr;
                       //         CLIENTS->queue = c;
                       //         ++CLIENTS->size; 
                       // }
                        ...
                        break;
                case 2:
                        printf("its a hay ack -- server shld never get this, error handle\n");
                ...
                case 3:
                        ...
                        printf("Server side check of list: %s\n",client_list); 
                ...
                case 5:
                        ...
                        cli_ptr = get(resp->source);  
                        ret_val = 1;
                        if(cli_ptr == NULL) {
                               // ret_val = 1;
                                break;
                        }
                        cli_ptr->mess_to_send = 1;
                        cli_ptr->header = resp;                                        
                ...
                case 6:
                        ...
                        struct sock_cli *ptr = CLIENTS->queue[sock];
                        CLIENTS->queue[sock] = NULL;
                        free(ptr);
                        
                        ret_val = 2;
                        break;
                case 7:
                        printf("is a cli already here error NEED ERROR HANDLE\n");
                        //error function TO DO
                        ret_val = 1;
                        break;
                case 8:
                        printf("is a cant deliver error NEED ERROR HANDLE\n");
...
void rec_message(struct header_C *resp, int sock) {                        
        ...
        //struct sock_cli *sock_ptr = CLIENTS->queue[sockfd];
                        
        struct cli_ID *ptr = get(resp->dest);
        int dest_sock = 0;
        if(ptr == NULL) {
                printf("rec_message needs error handling for NO SUCH DESTINATION CLIENT\n");  
                ...
                printf("Destination check: %s %d %d\n",ptr->ID, ptr->sockfd, dest_sock);
...
struct cli_ID *check_for_message_in(int sock){
        ...
        struct sock_cli *sock_ptr;
        struct cli_ID *ptr;
        if(CLIENTS->size == 0) {
                printf("check mess in func: client queue is empty\n");
                return NULL;
        } else {
                printf("blerghhh\n");
                sock_ptr = CLIENTS->queue[sock];
                if(sock_ptr != NULL) {                
                        ptr = get(sock_ptr->ID);
                        if(ptr != NULL) {
                                ptr->mess_to_send = 0;
...
int check_for_message_out(int sock){
        ...
        int n, del = 0, retval = 0;
        struct message_C *ptr = MESSAGE_QUEUE->queue;
        struct message_C *del_ptr;
        struct message_C *queue_ptr = MESSAGE_QUEUE->queue;
        while(ptr != NULL){
                del = 0;
                if(sock == ptr->sockfd){
...
void return_clients(char *cli_list) {
        ...
        for(i = 0; i < 30; ++i){
                if(CLIENTS->queue[i] != NULL){
                        memcpy(&cli_list[bytes],CLIENTS->queue[i]->ID,strlen(CLIENTS->queue[i]->ID));
                        bytes += strlen(CLIENTS->queue[i]->ID);
                }
        }
        return;
}       
...
struct cli_ID *get(char *key) {
        ...
        if(strcmp(ptr->ID, key) != 0) {
                printf("hash table GET error %s %s %d\n", key, ptr->ID, index);
                return NULL;
        }
        return ptr;
