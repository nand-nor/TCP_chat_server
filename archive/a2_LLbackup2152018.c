...
int parse_resp(int sock, struct header_C *resp, char *buffer) {
        ...
        switch(type) { 
                ...
                case 5:
                        ...
                        //struct cli_ID *src = get(resp->source);
                        //struct cli_ID *dest = get(resp->dest);
                        ...
                        while(ptr != NULL){
                                if(ptr->sockfd == sock) {
                                        ...
                                        //printf("cli queue %s versus get func %s", ptr->ID, src->ID);
                                        //printf("dest ID: %s\n", dest->ID);
...
int check_for_message_out(int sock){
        ...
        if(sock == ptr->sockfd){
                printf("theres a message in the queue for you!\n");
